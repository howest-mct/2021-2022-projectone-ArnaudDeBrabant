...
class MPU6050:
    ...
    def accel_range(self, value):
        if 0 <= value < 4:
            # accel scale range 2g
            self.i2c.write_byte_data(self.addr, 0x1C, value << 3)
    ...
    def gyro_range(self, value):
        if 0 <= value < 4:
            # accel scale range 2g
            self.i2c.write_byte_data(self.addr, 0x1B, value << 3)
    ...
    def __setup(self):
        ...
        # accel scale range 2g
        self.i2c.write_byte_data(self.addr, 0x1C, self.accel_range << 3)
        # Gyro scale range 250
        self.i2c.write_byte_data(self.addr, 0x1B, self.gyro_range << 3)
