...
class SpelBeheer:
    huidigSpel = None
    RFIDtags = ["66 c0 54 73", "52 ae 43 73", "6a 08 15 a3",
                "29 bf fd a3", "7a 63 50 73", "2a 1f 56 73"]
    ...
    def spel_maken(begintijd, eindtijd, spelers):
        ...
        # - Begintijd, Eindtijd
        SpelID = DataRepository.spel_aanmaken(begintijd, eindtijd)
        if (SpelID != None):
            print(SpelID)
            index = 0
            for speler in spelers:
                try:
                    ...
                    RFID = None
                SpelerID = DataRepository.nieuwe_speler(RFID, speler)
                DataRepository.speler_toevoegen(SpelID, SpelerID)
            return SpelID
        else:
            return None
